function [V_out] = Golomb_2007_RHS(t,V_in,no_cells,g_struct,I_app,I_on,noise_multiplier)
    ...
    Vs_out = g_struct.gNa.*(steady(Vs_in,g_struct.theta_m,sigma_m).^3).*h_in.*(ENa-Vs_in) + g_struct.gK.*(n_in.^2).*(EK-Vs_in) ...
      + g_struct.gD.*a_in.^3.*b_in.*(EK-Vs_in) + g_struct.gL.*(ERest-Vs_in) + I_app.*(t > I_on) + noise_multiplier.*randn(no_cells,1); %real(sqrt(2*noise_multiplier*randn));    %Update I-cell voltage.

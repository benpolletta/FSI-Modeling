...
function [V,s,t] = stochastic_ing(gI,no_cells,lambda,no_inputs,T0)

%   lambda = 0.01;                   %Input rate (per millisecond).
  tauI = 10;                        %Inhibitory time constant (in milliseconds).
  ...
  AMPA_in = poiss_AMPA_input(dt,T0,lambda,no_inputs); %Compute excitatory inputs.
  ...
  V(:,1)=-70.0 + 10*rand(no_cells,1);  	%Set the initial conditions for P-cells, I-cells, and synapses.
  m(:,1)=0.05 + 0.1*rand(no_cells,1);
  h(:,1)=0.54 + 0.1*rand(no_cells,1);
  n(:,1)=0.34 + 0.1*rand(no_cells,1);
  s(:,1)=0.0 + 0.1*rand(no_cells,1);
  
  for i=1:T-1                       %Integrate the equations.
      
      %I-cell dynamics - NOTE the synaptic current!
      V(:,i+1) = V(:,i) + dt*(gNa*(m(:,i).^3).*h(:,i).*(ENa-(V(:,i)+65)) + gK*(n(:,i).^4).*(EK-(V(:,i)+65)) + gL*(ERest-(V(:,i)+65)) ...
          - CE*AMPA_in(:,i).*V(:,i) + CI*s(:,i).*(-80-V(:,i)));                                                                                 %Update I-cell voltage.

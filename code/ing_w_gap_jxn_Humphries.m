...
%  CS = inhibitory synapse connectivity matrix.
%  CG = gap junction connectivity matrix.
...
function [V,s,t] = ing(I0,gI,tauI,T0,CS,CG)
  ...
  V = zeros(T,no_cells);                   %Make empty variables to hold I-cell results.
  m = zeros(T,no_cells);
  h = zeros(T,no_cells);
  n = zeros(T,no_cells);

  s = zeros(T,no_cells);                   %Make empty variables to hold the synapse results.
  ...
  for i=1:T-1                       %Integrate the equations.
      ...
      V(i+1,:) = V(i,:) + dt*(gNa*m(i,:)^3*h(i,:)*(ENa-(V(i,:)+65)) + gK*n(i,:)^4*(EK-(V(i,:)+65)) + gL*(ERest-(V(i,:)+65)) + I0 ...
          + gI*s(i,:)*(-80-V(i,:))) + gG*ones(1,no_cells)*(G_mat-CG*diag(V(i,:)));                           %Update I-cell voltage.
      ...
      G_mat_old = reshape(G(i,:),no_cells,no_cells);																%Update G.
      V_diff = CG*diag(V(i,:)) - G_mat_old;
      G_mat_new = G_mat_old + dt*V_diff.*V_diff'/tau_G;
      G(i+1,:) = reshape(G_mat_new,1,no_cells^2);

...
function [V,s,t] = stochastic_ing_w_gap_jxns(no_cells,gI,pI,gG,pG,no_inputs,lambda,T0)
...
%   lambda = 0.005;
  ...
  for i=1:T-1                       %Integrate the equations.
      ...
      V(:,i+1) = V(:,i) + dt*(gNa*(m(:,i).^3).*h(:,i).*(ENa-(V(:,i)+65)) + gK*(n(:,i).^4).*(EK-(V(:,i)+65)) + gL*(ERest-(V(:,i)+65)) ...
          - CE*AMPA_in(:,i).*V(:,i) + CI*s(:,i).*(-80-V(:,i)) + (CG*diag(V(:,i))-diag(V(:,i))*CG)*ones(no_cells,1));                                                                                 %Update I-cell voltage.

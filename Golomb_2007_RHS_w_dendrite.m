function [V_out] = Golomb_2007_RHS_w_dendrite(t,V_in,no_cells,CS,CG,theta_m,gD_in,I_app,I_on,noise_multiplier)
    ...
    n_in = V_in(3*no_cells + (1:no_cells));
    a_in = V_in(4*no_cells + (1:no_cells));
    b_in = V_in(5*no_cells + (1:no_cells));
    s_in = V_in(6*no_cells + (1:no_cells));

    Vs_out = gNa*(inf(Vs_in,theta_m,sigma_m).^3).*h_in.*(ENa-Vs_in) + gK*(n_in.^2).*(EK-Vs_in) ...
      + gD.*a_in.^3.*b_in.*(EK-Vs_in) + gL*(ERest-Vs_in) + I_app*(t > I_on) + noise_multiplier*randn(no_cells,1) ...
      + CS*s_in.*(-80-Vs_in) + g_sd*(Vd_in-Vs_in);                                                      %Update voltage of soma.
    Vd_out = (gNa/10)*(inf(Vs_in,theta_m,sigma_m).^3).*h_in.*(ENa-Vs_in) + (gK/10)*(n_in.^2).*(EK-Vs_in) ...
      + (gD/10).*a_in.^3.*b_in.*(EK-Vs_in) + (gL/10)*(ERest-Vs_in) ...

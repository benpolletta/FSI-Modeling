function [Vs_all,ISIm,gsd,F]=dendritic_conductance_sweep(gsd_range,no_secs,no_steps,no_reps)
...
% no_steps=5;
gsd_step=(gsd_range(2)-gsd_range(1))/no_steps;

gsd=zeros(1,no_steps+1);
...
for r=1:no_reps
    ...
    parfor i=1:(no_steps+1)
        ...
        gsd_range_temp=gsd_range;
        f_temp=f;
        gsd_temp=gsd_range_temp(1)+gsd_step*(i-1);
        
        [Vs,~,~,~] = ing_w_dendritic_gap_jxn(2,100,no_secs*1000,gsd_temp,0,[0 1; 1 0]*.1);
...
figure()
scatter(reshape(gsd,no_reps*(no_steps+1),1),reshape(ISIm,no_reps*(no_steps+1),1))
xlabel('Somato-Dendritic Conductance')
ylabel('(Mean Inter-Cell ISI)/(Mean Intra-Cell ISI)')
saveas(gcf,['gsd_',num2str(gsd_range(1)),'to',num2str(gsd_range(2)),'.fig'])

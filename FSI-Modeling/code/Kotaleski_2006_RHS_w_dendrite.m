function [V_out] = Kotaleski_2006_RHS_w_dendrite(t,V_in,no_cells,CS,CG,theta_m,gD_in,I_app,I_on,noise_multiplier)
    ...
    Vd_out = (gNa/10)*(inf(Vs_in,theta_m,sigma_m).^3).*h_in(no_cells+(1:no_cells)).*(ENa-Vs_in) + (gK/10)*(n_in(no_cells+(1:no_cells)).^2).*(EK-Vs_in) ...
      + (gD/10).*a_in(no_cells+(1:no_cells)).^3.*b_in(no_cells+(1:no_cells)).*(EK-Vs_in) + (gL/10)*(ERest-Vs_in) ...

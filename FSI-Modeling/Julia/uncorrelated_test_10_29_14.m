...
for j = 1:max_j
    ...
    for k = 1:max_k
		...
		[Vs,Vd,s,m,h,n,t] = ing_w_dendritic_gap_jxn(no_cells, epsps-ipsps, T0, [], CS, (k-1)*CG);
...
firing_rate = (firing_rate/no_cells)*(1000/T0);
firing_avg = mean(firing_rate, 2);

spike_pairs = spike_pairs./(firing_rate*(T0/1000)); %element-wise
pair_avg = mean(spike_pairs, 2);

gj_intervals = (0:gj_strength:(max_k-1)*gj_strength);
figure
plot(gj_intervals,firing_avg)

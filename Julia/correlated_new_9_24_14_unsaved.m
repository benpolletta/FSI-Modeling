...
t = (1:T)*dt;
no_cells = 10;
p_gj = 0.3; %probability of gap junction between any pair of cells. should be 1 if you're using 2 cells

no_e_inputs = 127; %127 = number of AMPA input synapses per cell in Hjorth et al, times 10 cells
...
max_k = 5; %number of correlation values to use

gap_conductance = (0.03/no_cells);
...
for j = 1:max_j
    ...
    e_spikes_indep = rand(no_e_inputs*no_cells, length(t));
    e_spikes_indep = e_spikes_indep < e_rate*dt/1000;
    
    e_spikes_shared = rand(no_e_inputs, length(t));
    e_spikes_shared = e_spikes_indep < e_rate*dt/1000;
    ...
    i_spikes_indep = rand(no_i_inputs*no_cells,length(t));
    i_spikes_indep = i_spikes_indep < i_rate*dt/1000;
    
    i_spikes_shared = rand(no_i_inputs,length(t));
    i_spikes_shared = i_spikes_shared < i_rate*dt/1000;
    ...
    i_spike_arrivals = CE_i*i_spikes_indep; % Calculating presynaptic spikes for each cell.
    ...
    e_spike_arrivals = CE_e*e_spikes_indep; % Calculating presynaptic spikes for each cell.

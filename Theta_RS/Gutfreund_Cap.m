...
model_eqns = ['dv/dt=(I_const+@current)/Cm; Cm=.9; v(0)=-65;',... % +I(t)
    ...
    sprintf('ton=%g; toff=%g; I_app=C_mult*%g;', ton, tspan, I_app),... %  (ton<t&t<toff) %%% 'PPstim = 0; PPfreq = 1.5; PPwidth = floor((1000/PPfreq)/4); PPshift = 0; ap_pulse_num = 0; kernel_type = 7;',... % in ms % 'noise=.25; I(t)=C_mult*I_app*((t/ton)*(t<=ton)+(ton<t&t<toff))*(1+rand*noise);',... % *((1-pulse/2)+pulse*(mod(t,750)<250&t>2*ton));',...
    'monitor functions'];

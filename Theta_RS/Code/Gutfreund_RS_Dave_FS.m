function [data, name] = Gutfreund_RS_Dave_FS(Iconst, tspan, save_flag, Nrs, Nfs, varargin)
...
name = ['gutfreund_RS_dave_FS', vary_label];
...
spec.populations(1).equations = ['dv/dt=(Iconst+@current)/Cm; Cm=1.8; v(0)=-65;',... % +I(t)
    'gNaP_denom=3.36; gNaP=gKs/gNaP_denom;',...
    sprintf('Iconst=%g;', Iconst),...    %  halfKs=-60; halfNaP=-60; 
    'tau_fast=5; tauh=tau_fast; taum=tau_fast;',... %%% 'offset=0; Koffset=offset; Noffset=offset;',... %'slow_offset=0; halfKs=-35-slow_offset; halfNaP=-40-slow_offset;',... 'fast_denom=1; gKDR=23.4/fast_denom; gNa=45/fast_denom;',... % C_mult*ones(1, 2)),...
    'monitor functions'];
spec.populations(1).mechanism_list = {'iNaP','iKs','iNaG','iKDRG','gleak','CaDynT','iCaT','iKCaT','iStep'};
spec.populations(1).parameters = {...
    'gKs', .4644, 'gl', .1044, 'gCa', .072, 'gKCa', .018, 'bKCa', .002, 'tau_div', 2,... % C_mult*ones(1, 3)),... %%% 
...
if Nrs > 1
    ...
    spec.connections(i).parameters = {'gSYN',gAMPA_rsrs,'ESYN',EAMPA,'tauDx',tauAMPAd, ...
        'tauRx',tauAMPAr,'fanout',inf,'ICnoise',0,'gSYNhetero',gSYNhetero, ...
...
if Nrs > 0 && Nfs > 0
    ...
    spec.connections(i).parameters = {'gSYN',gAMPA_rsfs,'ESYN',EAMPA,'tauDx',tauAMPAd, ...
        'tauRx',tauAMPAr,'fanout',inf,'ICnoise',0,'gSYNhetero',gSYNhetero, ...
...
if Nfs > 0
    ...
    spec.connections(i).parameters = {'gSYN',gGABAaff,'ESYN',EGABA,'tauDx',tauGABAad,...
        'tauRx',tauGABAar,'fanout',inf,'ICnoise',0,'gSYNhetero',gSYNhetero,...
        'g_GAP',ggjFS,...
        };
end

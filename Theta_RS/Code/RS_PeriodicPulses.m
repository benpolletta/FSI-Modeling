function [data, name] = RS_PeriodicPulses(Iconst, tspan, save_flag, varargin)
...
model_eqns = ['dv/dt=(Iconst+@current)/Cm; Cm=2.7; v(0)=-65;',...
    ...
    sprintf('gKs=Cm_factor*0.124; gNaP_denom=3.36; gNaP=gKs/gNaP_denom; Iconst=%g;', Iconst),...    %  halfKs=-60; halfNaP=-60; gKs=0.084; gNaP=0.025; gl=0.025; gCa=0.02; %%% 
    'bKCa=.002; tau_fast=5; tauh=tau_fast; taum=tau_fast;',... % 'slow_offset=0; slow_offset_correction=0; Ks_offset=slow_offset-slow_offset_correction; NaP_offset=slow_offset;',... % 'fast_offset=0; Koffset=fast_offset; Noffset=fast_offset;',...     % 'fast_denom=1; gKDR=5/fast_denom; gNa=12.5/fast_denom;',... % 'I(t)=I_app*((t/ton)*(t<=ton)+(ton<t&t<toff)+rand*.1);',... % *((1-pulse/2)+pulse*(mod(t,750)<250&t>2*ton));',...

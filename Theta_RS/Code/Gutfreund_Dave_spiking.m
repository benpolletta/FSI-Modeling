function [data, name] = Gutfreund_Dave_spiking(Iconst, tspan, save_flag, varargin)
...
model_eqns = ['dv/dt=(Iconst+@current)/Cm; Cm=.9; v(0)=-65;',... % +I(t)
    ...
    'gl=.1044; gCa=.072; gKCa=.018; bKCa=.002; tau_div=2;',... % C_mult*ones(1, 3)),... %%% 
    'tau_fast=0; NaF_c0=max(tau_fast,.15); KDR_c0=max(tau_fast,.25); NaF_c1=1.3-tau_fast; KDR_c1=4.6-tau_fast;',... %%% 'offset=0; Koffset=offset; Noffset=offset;',...
    'slow_offset=0; halfKs=-35-slow_offset; halfNaP=-40-slow_offset;',...
    'fast_offset=0; offset_correction=0; NaF_offset=fast_offset; KDR_offset=fast_offset-offset_correction;',...
    'fast_denom=1; gKDR=23.4/fast_denom; gNaF=45/fast_denom; EKDR=-95; ENaF=50;',... % C_mult*ones(1, 2)),...

function results = Gutfreund_KCa_PRC_plot(data, results, name)
...
if isempty(results)
   ...
   results = dsAnalyze(data, @Gutfreund_PRC_metrics);
    
end

stim = [data.pop1_STPstim]';
stims = unique(stim);

shift = [data.pop1_STPshift]';
shifts = unique(shift);

% rand = [data.pop1_randParam];
% rands = unique(rand);
